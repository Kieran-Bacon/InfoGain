...
class Datapoint:
    ...
    def __str__(self):
        ...
        scores = [score for score in [self.annotation, self.prediction, self.probability] if score]
    ...
    def __hash__(self):
        return hash(self.domain) + hash(self.target) + hash(self.relation) + hash(self.text)    
