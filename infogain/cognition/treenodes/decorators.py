...
def scenario_consistent(function):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if "scenario" not in kwargs:
            raise ConsistencyError("No Scenario was passed through the evaluation tree for {} - {}".format(
                type(self), self))
        ...
        if hasattr(self, "__scenario_requires__"):
            ...
            if missing:
                raise ConsistencyError("Scenario is missing required entry for {} {}: missing: {}".format(
                    type(self), self, missing))                    
